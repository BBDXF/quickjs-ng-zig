/*
 * Copy qjs.c from quickjs-ng
 * change the main function to run in myui
 */
#include <stdlib.h>
#include <stdio.h>
#include <stdarg.h>
...
static int64_t parse_limit(const char *arg) {
  ...
  if (*p) {
    ...
    default:
      fprintf(
          stderr,
          "Invalid limit: %s, unrecognized suffix, only k,m,g are allowed\n",
          arg);
      return -1;
    }
    if (*p) {
...
static JSValue js_navigator_get_userAgent(JSContext *ctx,
...
static const JSCFunctionListEntry navigator_proto_funcs[] = {
    ...
    JS_PROP_STRING_DEF("[Symbol.toStringTag]", "Navigator",
...
static const JSMallocFunctions trace_mf = {js_trace_calloc, js_trace_malloc,
                                           js_trace_free, js_trace_realloc,
...
//            "-C  --script       load as JS classic script
//            (default=autodetect)\n"
//            "-m  --module       load as ES module (default=autodetect)\n"
//            "-I  --include file include an additional file\n"
//            "    --std          make 'std', 'os' and 'bjson' available to
//            script\n"
//            "-T  --trace        trace memory allocation\n"
//            "-d  --dump         dump the memory usage stats\n"
//            "-D  --dump-flags   flags for dumping debug data (see DUMP_*
//            defines)\n"
//            "-c  --compile FILE compile the given JS file as a standalone
//            executable\n"
//            "-o  --out FILE     output file for standalone executables\n"
//            "    --exe          select the executable to use as the base,
//            defaults to the current one\n" "    --memory-limit n       limit
//            the memory usage to 'n' Kbytes\n" "    --stack-size n limit the
//            stack size to 'n' Kbytes\n"
...
JS_App *js_app_new(int max_stack_size, int max_heap_size) {
  JS_App *app = malloc(sizeof(JS_App));
  memset(app, 0, sizeof(JS_App));
  // init
  app->rt = JS_NewRuntime();
  if (max_stack_size > 0)
    JS_SetMaxStackSize(app->rt, max_stack_size);
  if (max_heap_size > 0)
    JS_SetMemoryLimit(app->rt, max_heap_size);

  // context
  js_std_set_worker_new_context_func(JS_NewCustomContext);
  js_std_init_handlers(app->rt);
  app->ctx = JS_NewCustomContext(app->rt);
...
int js_app_run(JS_App *app, const char *data, const int data_len, const char *name, bool is_module) {
  if (app == NULL || data == NULL) {
    return -1;
  }
  
  return eval_buf(app->ctx, data, data_len, name, is_module? JS_EVAL_TYPE_MODULE : JS_EVAL_TYPE_GLOBAL);
}
...
void js_app_eval_bin(JS_App* app, const char* data, const int data_len){
  ...
  js_std_eval_binary(app->ctx, data, data_len, 0);
}
